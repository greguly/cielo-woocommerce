...
class WC_Cielo_Credit_Installment_Change {
    ...
    public function cart_calculate_fees( $cart ) {
        ...
        if (WC()->session->chosen_payment_method == 'cielo_credit') {
        ...
        } elseif (WC()->session->chosen_payment_method == 'cielo_debit') {

            $settings = get_option('woocommerce_' . WC()->session->chosen_payment_method . '_settings');

            if (isset($settings['debit_discount'])) {

                // Gets the gateway discount.
                $debit_discount = (float) $settings['debit_discount'];

                if (0 < $debit_discount) {

                    $total = (WC()->cart->cart_contents_total + WC()->cart->shipping_total);

                    $discount = $total * ($debit_discount / 100);
                    $cart->add_fee(
                        sprintf(__('With discount of %s at debit card', 'cielo-woocommerce'), $debit_discount . '%', sanitize_text_field(woocommerce_price($total))),
                        -$discount
                    );
